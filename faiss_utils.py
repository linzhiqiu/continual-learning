...
def path_iterator_for_numpy(paths, mapping):
...
def get_total_feature_length(paths):
    feature_count = 0
    for path in paths:
        matrix = load_pickle(path)
...
class KNearestFaissFeatureChunks():
    def __init__(self, clip_features_normalized_paths, model, preprocess):
        ...
        self.total_feature_num = get_total_feature_length(self.clip_features_normalized_paths)
    ...
    def grab_bottom_query_indices(self, query, start_idx=0, end_idx=2000):
        ...
        D, indices = self.k_nearest(normalize_text_feature, k=end_idx)
    ...
    def k_nearest(self, feature, k=4):
        ...
        for chunk in chunk_iter:
            ...
            D, I = knn_ground_truth(feature, path_iterator_for_numpy(self.clip_features_normalized_paths, mapping), size_chunk)
            end_search = time.time()
            all_D += list(D[0])
