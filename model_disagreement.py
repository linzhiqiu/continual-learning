...
def split_dataset(query_dict, TRAIN_SET_RATIO=TRAIN_SET_RATIO, TEST_SET_RATIO=TEST_SET_RATIO):
    ...
    for query in all_query:
        ...
        dataset_dict[query]['train_set'] = gather_data(
            query, train_set_indices)
    ...
    if NEGATIVE_LABEL in query_dict:
        ...
        dataset_dict[NEGATIVE_LABEL]['train_set'] = gather_data(
            NEGATIVE_LABEL, train_set_indices)
        dataset_dict[NEGATIVE_LABEL]['test_set'] = gather_data(
            NEGATIVE_LABEL, test_set_indices)
        dataset_dict[NEGATIVE_LABEL]['all'] = gather_data(
            NEGATIVE_LABEL, data_indices)
    #
    return dataset_dict
...
def get_all_loaders_from_dataset_dict(all_dataset_dict, hyperparameter, excluded_bucket_idx=0):
    ...
    dataset_dict = {k: all_dataset_dict[k]
        ...
        for k_name in ['train_set', 'test_set']:
            ...
            if feature_name == 'metadata':
                loader = make_image_loader(
                    items, hyperparameter, shuffle=shuffle)
...
def train(train_loader, test_loader,
    ...
    if train_mode in TRAIN_MODES_CATEGORY['nearest_mean']:
        ...
        for label_i in network:
            network[label_i] = network[label_i] / \
                np.linalg.norm(network[label_i])
        ...
        for epoch in range(0, epochs):
            ...
            for phase in loaders.keys():
                ...
                for batch, data in enumerate(pbar):
                    ...
                    if train_mode in TRAIN_MODES_CATEGORY['feature_normalized']:
                        inputs = inputs / \
                            torch.linalg.norm(inputs, dim=1, keepdim=True)
                    labels = labels.cuda()
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if phase == 'train':
                            ...
                            if train_mode in TRAIN_MODES_CATEGORY['projected_normalized']:
                                network.weight.data = network.weight.data / \
                                    torch.linalg.norm(
                                        network.weight.data, dim=1, keepdim=True)
                            elif train_mode in TRAIN_MODES_CATEGORY['projected_normalized_true']:
                                msk = torch.linalg.norm(
                                    network.weight.data, dim=1) < 1.
                                network.weight.data[msk] = (
                                    network.weight.data/torch.linalg.norm(network.weight.data, dim=1, keepdim=True))[msk]
                ...
                if phase == 'train':
                    ...
                    if avg_acc > best_test_acc:
                        print(
                            f"Best test accuracy at epoch {epoch} being {avg_acc}")
                        best_test_epoch = epoch
                        best_test_acc = avg_acc
                        best_test_epoch_train_acc = avg_acc_per_epoch[-1]
                        print(
                            f"Train accuracy at epoch {epoch} being {best_test_epoch_train_acc}")
