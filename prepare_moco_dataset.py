...
# python prepare_moco_dataset.py --model_name RN50 --class_size 1000 --num_of_bucket 3 --query_title none
# python prepare_moco_dataset.py --model_name RN50 --class_size 1000 --num_of_bucket 3 --query_title photo

# python prepare_moco_dataset.py --model_name RN50 --class_size 1000 --num_of_bucket 10 --query_title none --folder_path /scratch/zhiqiu/yfcc100m_all/images_minbyte_10_valid_uploaded_date_feb_18 --clip_dataset_path /scratch/zhiqiu/yfcc100m_all/clip_dataset/images_minbyte_10_valid_uploaded_date_feb_18/clip_dataset_bucketed/
...
def get_clip_feature_folder_paths(folder_path, num_of_bucket):
    ...
    for b_idx in range(num_of_bucket):
        sub_folder_path = os.path.join(
            folder_path, f'bucket_{num_of_bucket}', f'{b_idx}')
...
def get_save_paths_by_bucket(clip_dataset_path, label_set, query_title, class_size, num_of_bucket):
    save_path = os.path.join(clip_dataset_path, label_set,
                             f"query_{query_title}_size_{class_size}_bucket_{num_of_bucket}")
...
if __name__ == '__main__':
    ...
    feature_name = get_feature_name(
        args.folder_path, args.model_name, normalize=True)
    ...
    date_uploaded_indices = [i[0] for i in sorted(
        enumerate(date_uploaded_list), key=lambda x: x[1])]
    sorted_indices_chunks = divide(date_uploaded_indices, args.num_of_bucket)

    bucket_dict = {}
    folder_paths = get_clip_feature_folder_paths(
        args.folder_path, args.num_of_bucket)
    for i, sorted_indices_chunk in enumerate(sorted_indices_chunks):
        min_date, max_date = date_uploaded_list[sorted_indices_chunk[0]
                                                ], date_uploaded_list[sorted_indices_chunk[-1]]
        date_str = f"For bucket {i}: Date range from {get_date_uploaded(min_date)} to {get_date_uploaded(max_date)}"
        print(date_str)

        clip_norm_features_location = get_feature_name(
            folder_paths[i], args.model_name, normalize=True)
        ...
        clip_features_normalized = np.concatenate(
            [f.reshape(1, -1) for f in feature_list], axis=0)
        save_obj_as_pickle(clip_norm_features_location,
        ...
        save_obj_as_pickle(os.path.join(
            folder_paths[i], f'bucket_{i}.pickle'), bucket_dict[i])
    ...
    for label_set in LABEL_SETS:
        ...
        queries = [QUERY_TITLE_DICT[args.query_title] +
                   label for label in labels]

        print(queries)
        print(f"We have {len(queries)} queries.")

        save_path, sub_save_paths = get_save_paths_by_bucket(
            args.clip_dataset_path, label_set, args.query_title, args.class_size, args.num_of_bucket)
        info_dict = {
        ...
        for b_idx in bucket_dict:
            ...
            k_near_faiss = KNearestFaiss(
                folder_paths[b_idx], args.model_name, flickr_accessor=bucket_dict[b_idx]['flickr_accessor'])
            ...
            def grab_top_query_images(query, start_idx=0, end_idx=40, diff_queries=[], lmb=0.):
                ...
                if len(diff_queries) != 0:
                    ...
                    normalize_text_feature = k_near_faiss.get_text_difference_feature(
                        query, diff_queries=diff_queries, lmb=lmb)
                else:
                    normalize_text_feature = k_near_faiss.get_normalized_text_feature(
                        query)
                end_feature = time.time()
                D, meta_list, clip_features = k_near_faiss.k_nearest_meta_and_clip_feature(
                    normalize_text_feature, k=end_idx)
                end_search = time.time()
                print(
                    f"{end_feature-start:.4f} for querying {query}. {end_search-end_feature} for computing KNN.")
            ...
            for idx, query in enumerate(queries):
                ...
                # else:
                D, query_meta_list, query_clip_features, text_feature = grab_top_query_images(
                    query, end_idx=args.class_size)
        ...
        def print_all(distances_dict, key='distance_mean'):
            ...
            for query in distances_dict:
                ...
                distances_dict_summary.append(
                    (distances_dict[query][key], query))
            distances_dict_summary = sorted(
                distances_dict_summary, key=lambda x: x[0], reverse=True)
        ...
        for query in queries:
            ...
            for b_idx in bucket_dict:
                distances_dict[query]['features_mean'].append(
                    query_dict[b_idx][query]['features_mean'])
            distance_dict_query = gather_distances(
                distances_dict[query]['features_mean'])
            distance_mean = print_distances(query, distance_dict_query)
            distance_first_last = gather_distances_first_last(
                distances_dict[query]['features_mean'])
