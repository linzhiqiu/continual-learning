...
from utils import save_obj_as_pickle, load_pickle, load_json
import threading
import sys
...
argparser.add_argument("--a",
                       default='/scratch/zhiqiu/yfcc100m_all_new_sep_21/images_minbyte_10_valid_uploaded_date_minedge_120_maxratio_2.0/',
                       )
...
class FlickrParser():
    ...
    def __init__(self, args, criteria : Criteria):
        ...
        self.hash_pickle = os.path.join(args.metadata_dir, 'yfcc100m', 'yfcc100m_hash.pickle')
...
if __name__ == "__main__":
    # python temp.py --a /scratch/zhiqiu/yfcc100m_all/images_minbyte_10_valid_uploaded_date_jan_31/
    # python temp.py --a /scratch/zhiqiu/yfcc100m_all/images_minbyte_10_valid_uploaded_date_feb_18/
    args = argparser.parse_args()
    folder = args.a
    folder_path = os.path.join(folder, 'all_folders.pickle')
    print(f"Working on folder {folder_path}")
    folders = load_pickle(folder_path)
    new_folders = {}
    from utils import save_as_json
    # import pdb; pdb.set_trace()
    if type(folders) == dict:
        for folder_key in tqdm(folders):
            new_folders[folder_key] = folders[folder_key].__dict__
            old_pickle_location = new_folders[folder_key]['metadata_location']
            if os.path.exists(old_pickle_location):
                one_folder = load_pickle(old_pickle_location)
            else:
                continue
            new_json_path = os.path.join(new_folders[folder_key]['folder'], "metadata.json")
            new_folders[folder_key]['metadata_location'] = new_json_path
            if os.path.exists(new_json_path):
                if not type(load_json(new_json_path)) == type(None):
                    continue
            try:
                new_meta_list = []
                for meta in one_folder:
                    new_meta = meta.metadata.__dict__
                    # del new_meta['metadata']
                    new_meta_list.append(new_meta)
                save_as_json(new_folders[folder_key]['metadata_location'], new_meta_list)
            except:
                import pdb; pdb.set_trace()
        
        save_as_json(os.path.join(folder, "all_folders.json"), new_folders)
    else:
        for folder_key, a_folder in tqdm(enumerate(folders)):
            new_folders[folder_key] = a_folder.__dict__
            old_pickle_location = new_folders[folder_key]['metadata_location']
            if os.path.exists(old_pickle_location):
                one_folder = load_pickle(old_pickle_location)
            else:
