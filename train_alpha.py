...
def get_two_buffer_loaders_from_features_dict(alpha_value_mode, alpha_value, features_dict, train_mode, hyperparameter, excluded_bucket_idx=0):
    all_bucket = sorted(list(features_dict.keys()))
    print(f"Excluding {excluded_bucket_idx} from the sequential loaders")
    if type(excluded_bucket_idx) == int:
        features_dict = {k: features_dict[k]
    ...
    loader_func = get_loader_func(train_mode, hyperparameter.batch_size)
    curr_buffer = [] # always store current samples
    memory_buffer = [] # store random samples from previous stream up to current samples
    ...
    for idx, b_idx in enumerate(all_bucket):
        ...
        if idx == 0:
            curr_buffer = features_dict[b_idx]['train'].copy()
            ...
            # Implement
            count_replace = 0
            for item in curr_buffer:
                rnd = random.random()
                if rnd <= prob:
                    count_replace += 1
                    rnd_idx = int(rnd * (previous_stream_size-1)) % len(memory_buffer)
                    memory_buffer[rnd_idx] = item
            print(f"{count_replace} items getting replaced out of {k} samples in memory_buffer")
            ...
            #     totalCount += 1
            #     new_elem = memory2[i]
            #     rnd = uniformInt(0, totalCount)
            #     if rnd <= k * alpha:
            #         memory1[int(rnd % k)] = new_elem
            #     else:
            #         ignore new_elem

            # memory2 = Strem(New-batch-of-k)

            # train_network_on (memory1 + memory2)

        
        train_loader = loader_func(curr_buffer.copy() + memory_buffer.copy(), True) # Important! TO use copy() otherwise it will be changed
...
def get_single_buffer_loaders_from_features_dict(alpha_value_mode, alpha_value, features_dict, train_mode, hyperparameter, excluded_bucket_idx=0):
    all_bucket = sorted(list(features_dict.keys()))
    print(f"Excluding {excluded_bucket_idx} from the sequential loaders")
    if type(excluded_bucket_idx) == int:
        features_dict = {k: features_dict[k]
...
if __name__ == '__main__':
    ...
    if args.alpha_buffer_mode == 'single_buffer':
        alpha_loader_func = get_single_buffer_loaders_from_features_dict
    else:
        alpha_loader_func = get_two_buffer_loaders_from_features_dict
    ...
    for alpha_value in ALPHA_VALUE_DICT[args.alpha_buffer_mode][args.alpha_value_mode]:
        ...
        alpha_loaders_dict_path = os.path.join(exp_result_save_path,
                                               f"alpha_loaders_dict_{dataset_str(args.mode)}_{args.alpha_buffer_mode}_{args.alpha_value_mode}_{alpha_value}_{args.train_mode}_{seed_str}_ex_{excluded_bucket_idx}.pickle")
        ...
        results_dict_alpha_retrain_path = os.path.join(exp_result_save_path,
                                                       f"results_dict_{args.alpha_buffer_mode}_{args.alpha_value_mode}_{alpha_value}_retrain_{dataset_str(args.mode)}_{args.train_mode}_{seed_str}_ex_{excluded_bucket_idx}.pickle")
        ...
        results_dict_alpha_finetune_path = os.path.join(exp_result_save_path,
                                                       f"results_dict_{args.alpha_buffer_mode}_{args.alpha_value_mode}_{alpha_value}_finetune_{dataset_str(args.mode)}_{args.train_mode}_{seed_str}_ex_{excluded_bucket_idx}.pickle")
