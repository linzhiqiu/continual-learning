...
class HyperParameter():
    def __init__(self, network_name, epochs=100, step=60, batch_size=256, lr=0.1):
    ...
    def get_detail_str(self):
        if self.network_name in TRAIN_MODES_CATEGORY['nearest_mean']:
            ...
            return "_".join([self.network_name, 'ep', self.epochs, 'step', self.step, 'b', self.batch_size, 'lr', self.lr])


HYPER_DICT = {
    'mlp': HyperParameter('mlp', epochs=10, step=10, batch_size=256, lr=0.003),
    'linear': HyperParameter('linear', epochs=10, step=10, batch_size=256, lr=0.003),
    'cnn': HyperParameter('cnn', epochs=10, step=2, batch_size=64, lr=0.1),
}

ALL_PRETRAINED_WEIGHTS = ['moco_yfcc_feb18_gpu_8_bucket_0', 'imgnet', 'moco_imgnet', 'byol_imgnet', None]
ALL_FEATURE_TYPES = ['clip', 'cnn_feature']
...
argparser.add_argument("--dataset_name",
                       default='dynamic_300_lbfgs', # best training loss + lbfgs
...
def train_lbfgs(loaders,
                train_mode, output_size,
                epochs=150, lr=0.1, step_size=60,
    ...
    optimizer = training_utils.make_lbfgs_optimizer(network, lr)
        ...
        for phase in phases:
            ...
            for batch, data in enumerate(pbar):
                ...
                with torch.set_grad_enabled(False):
                    ...
                    loss = criterion(log_probability, labels).item()
...
def run_baseline(all_loaders_dict, all_query, train_mode):
    result_baseline_dict = {'models': None, 
                            ...
                            'best_result' : None,
                            'avg_results' : None}
    all_model, all_accuracy, best_result, avg_results = train_lbfgs(all_loaders_dict,
                                                                    args.train_mode,
                                                                    len(all_query),
...
def run_single(loaders_dict, all_query, train_mode):
    ...
    for b1 in sorted_buckets:
        ...
        single_model, single_accuracy_b1, best_result, avg_results = train_lbfgs(loaders_dict[b1],
                                                                                 train_mode,
                                                                                 len(all_query),
                                                                                 epochs=HYPER_DICT[TRAIN_MODES_CATEGORY[train_mode].network_type].epochs,
                                                                                 lr=HYPER_DICT[TRAIN_MODES_CATEGORY[train_mode].network_type].lr,
