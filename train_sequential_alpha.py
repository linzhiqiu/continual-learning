...
def get_sequential_alpha_loaders_from_features_dict(alpha_value_mode, alpha_value, features_dict, train_mode, hyperparameter, excluded_bucket_idx=0):
    ...
    if alpha_value == None:
        ...
        for idx, b_idx in enumerate(all_bucket):
            ...
            alpha = 1.0
            b_idx_bucket = features_dict[b_idx]['train'].copy()
            for curr_idx, curr_b_idx in reversed(list(enumerate(all_bucket))):
                if curr_idx < idx:
                    print(f"For {idx} timestamp: Use {alpha} for {curr_idx}")
                    curr_bucket = features_dict[curr_b_idx]['train'].copy()
                    n += curr_bucket
                    p += [alpha for _ in curr_bucket]
            ...
            if len(p) != 0:
                ...
                train_items = np.random.choice(np.arange(len(n)), size=(len(n)), replace=True, p=p)
                train_items = [n[i] for i in train_items]
            else:
                print(f'No train_items for {b_idx}')
                train_items = []
            train_loader = loader_func(train_items.copy() + b_idx_bucket, True) # Important! TO use copy() otherwise it will be changed
            sequential_loaders_dict[b_idx]['train'] = train_loader
...
if __name__ == '__main__':
    ...
    for alpha_value in ALPHA_VALUE_DICT[args.alpha_value_mode]:
        ...
        alpha_loaders_dict_path = os.path.join(exp_result_save_path,
                                               f"cumulative_alpha_loaders_dict_{dataset_str(args.mode)}_{args.alpha_value_mode}_{alpha_value}_{args.train_mode}_{seed_str}_ex_{excluded_bucket_idx}.pickle")
        ...
        if os.path.exists(alpha_loaders_dict_path):
            ...
            alpha_loaders_dict = get_sequential_alpha_loaders_from_features_dict(
        ...
        results_dict_alpha_retrain_path = os.path.join(exp_result_save_path,
                                                       f"results_dict_cumulative_{args.alpha_value_mode}_{alpha_value}_retrain_{dataset_str(args.mode)}_{args.train_mode}_{seed_str}_ex_{excluded_bucket_idx}.pickle")
        ...
        results_dict_alpha_finetune_path = os.path.join(exp_result_save_path,
                                                       f"results_dict_cumulative_{args.alpha_value_mode}_{alpha_value}_finetune_{dataset_str(args.mode)}_{args.train_mode}_{seed_str}_ex_{excluded_bucket_idx}.pickle")
